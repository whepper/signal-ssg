...
| RSS (home + section, limit 20) | yes | main feed + per-term feeds from normalized data; `[feed]` opt-in with limit (default 20); section-list and label-list feeds deliberately out. Ordering contract: dated entries newest-first, undated last, ties broken by slug ascending then `ContentId` (`compare_by_date_desc`; BR-1 observed Hugo placing a same-date project page before an article where Signal applies the slug tie-break) | native | done |
| Taxonomy RSS (`topics/index.xml`, per-term) | yes | per-term feeds done (`<root>/<slug>/index.xml`); the label-list index feed is out (its rows are labels, not dated content) | native | partial |
| Sitemap / robots.txt / 404 | yes | sitemap done from the explicit route inventory; robots/404 out | sitemap native; rest slice 7+ | partial |
